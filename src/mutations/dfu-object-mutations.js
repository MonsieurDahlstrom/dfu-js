...
const TransferObjectMutations = {
  [MutationTypes.ADD_DFU_OBJECT] (state, transferObject) {
  ...
  [MutationTypes.UPDATE_DFU_OBJECT] (state, transferObject) {
    const objectIndex = state.objects.indexOf(transferObject)
    if (objectIndex >= 0) {
      state.objects.splice(objectIndex,1,transferObject)
    }
  },
  [MutationTypes.REMOVE_DFU_OBJECT] (state, transferObject) {
